...
timestamp="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
{
  ...
  if [ -n "$hook_input" ]; then
    echo "timestamp=$timestamp hook=codex.stop stdin_json_begin"
    printf '%s\n' "$hook_input"
    echo "timestamp=$timestamp hook=codex.stop stdin_json_end"
  fi
  cd "$ROOT" || exit 1
  ...
  if [ "$done_status" -eq 0 ] && [ "$status_status" -eq 0 ]; then
    ...
    if [ "$done_status" -ne 0 ]; then
      echo "timestamp=$(date -u +"%Y-%m-%dT%H:%M:%SZ") hook=codex.stop handoff=not_generated reason=done_failed"
    fi
  fi
} >> "$LOG" 2>&1
