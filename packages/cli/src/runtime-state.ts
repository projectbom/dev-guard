...
function renderAgentContext(input: {
  ...
  return [
    ...
    "",
    "### New Task",
    "1. Call DevGuard MCP `prepare_task_context` with the current user request.",
    "2. Open the returned highest-priority file and line range first.",
    "3. Expand only when callers, dependencies, routes, schemas, tests, or data flow require it.",
    `4. Use \`${devguardPaths.agentBrief}\`, \`${devguardPaths.readMap}\`, and \`${devguardPaths.codeMap}\` only when MCP is unavailable or its result is insufficient.`,
    `5. Do not treat \`${devguardPaths.projectHandoff}\` as the source of truth for a new task.`,
    "",
    "### Resumed Task",
    `1. Read \`${devguardPaths.projectHandoff}\` only when the user explicitly asks to continue previous work.`,
    "2. Convert the handoff next action into a concrete task.",
    "3. Call DevGuard MCP `prepare_task_context` with that concrete task.",
    "4. Continue from the returned file and range recommendations.",
    "",
    "### MCP Fallback",
    ...
    "",
    "## Handoff Scope",
    `- previous-session handoff: \`${devguardPaths.projectHandoff}\``,
    "- use it by default only for explicitly resumed work",
    "- for new tasks, call `prepare_task_context` first and treat the user request as authoritative",
    "",
    "## Important Decisions",
...
function renderNextClaudePrompt(input: { qualityVerdict: string; nextBestTask: string }): string {
  return [
    ...
    "For a new task:",
    "",
    "1. Call DevGuard MCP `prepare_task_context` with the user's current request.",
    "2. Open the returned highest-priority file and line range first.",
    "3. Expand only when dependencies or data flow require it.",
    "",
    "For explicitly resumed work:",
    "",
    `1. Read \`${devguardPaths.projectHandoff}\`.`,
    "2. Convert the next action into a concrete task.",
    "3. Call `prepare_task_context`.",
    "",
    "MCP fallback:",
    "",
    `1. \`${devguardPaths.agentBrief}\` — task intent and constraints`,
    ...
    `4. \`${devguardPaths.workingContext}\` — structural background only when needed`,
    "",
    "Do not use project handoff as the source of truth for a new task.",
    "Do not perform repository-wide scans when DevGuard returns relevant candidates.",
    "",
    "---",
    "",
...
function renderNextPrompt(input: {
  ...
  return [
    ...
    `Use \`${devguardPaths.projectHandoff}\` as the authoritative next action only for explicitly resumed work.`,
    "",
    "## Next",
    "- New task: call `prepare_task_context`, then start from the returned files and line ranges.",
    `- Resumed task: read \`${devguardPaths.projectHandoff}\`, convert its next action into a concrete task, then call \`prepare_task_context\`.`,
    `- MCP fallback: use \`${devguardPaths.agentBrief}\`, \`${devguardPaths.readMap}\`, and \`${devguardPaths.codeMap}\` in that order.`,
    ...
    `- resumed-work source: ${devguardPaths.projectHandoff}`,
...
function renderProjectHandoff(input: {
  ...
  body.push(
    input.locale === "ko-KR"
      ? "> 이 문서는 이전 DevGuard 세션을 재개하기 위한 인수인계입니다. 새 작업은 DevGuard MCP `prepare_task_context`를 먼저 호출하세요."
      : "> This report is for resuming the previous DevGuard session. For a new task, call DevGuard MCP `prepare_task_context` first.",
    ""
  );
